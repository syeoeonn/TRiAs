...
def any_dual_filter_condition(case_ratios, ctrl_ratios, thr=2.0):
...
def mc_ratio_cmd(args):
    ...
    for _, row in df.iterrows():
        ...
        if args.dual_filter:
            ...
            flag_total.append("Y" if any_dual_filter_condition(r_total, ur_total, args.threshold) else "N")
            flag_maxc.append("Y" if any_dual_filter_condition(r_maxc, ur_maxc, args.threshold) else "N")
        else:
    ...
    # Add columns in fixed order (same as original code)
